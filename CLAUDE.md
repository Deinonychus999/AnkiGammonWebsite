...
- Checker plays are answered by moving the checkers on the board, with OpenGammon's mechanics (Settings can switch back to picking from a list). [js/train-board.js](website/public/js/train-board.js) is the playable board: tapping a checker plays one die (right click the other), tapping an empty point or a blot brings up to two checkers, tapping the tray bears off, dragging past 12px (18px by touch) shows the reachable points; the dice dim as they're used, swap on a click, and submit a complete move on a click or Space; Undo is drawn on the board. Moves take effect at once and glides only show them, so fast taps are never lost and glides overlap. [js/train-moves.js](website/public/js/train-moves.js) (no DOM) generates the legal plays for a roll, applies XG move notation, and writes a play back in it; the played position is matched against every analysed candidate's position, and a move the analysis doesn't list is graded as worse than the worst listed one. `BoardRenderer.geometry()` gives where checkers, dice and the tray sit, for drawing on top of a rendered board. After answering, the board is a display-only `TrainBoard` (`display: true`) whose `show(pos)` replays the legal play ending in `pos` from the start position: the Start / Your move / Best move buttons and every row of the analysis table (click, Enter, or the arrow keys) use it, and the winning chances follow the move shown. The Equity header flips cubeful/cubeless and the score matrix has an Errors / Equities switch; both choices are kept in `settings` (`equity`, `matrixValues`).
- `/train/#desktop=<port>.<key>`: the desktop app's Study in Trainer serves one pack at `http://127.0.0.1:<port>/pack/<key>` for about two minutes; the trainer fetches it (Chrome and Edge ask about local-network access; Safari can't), falls back to pointing at File → Export to Trainer, and once the deck is saved sends `POST /done/<key>` so the desktop app stops serving it. Chrome can deliver the GET before the user answers its prompt, which is why the desktop app waits for that receipt instead of counting the GET.
- Pack positions may carry `extras` from the desktop app's Send to Trainer export: `score_matrix` (cells of `analysis/score_matrix.ScoreMatrixCell` via `asdict`, `current`, `caption`, `projection`, `unlimited`), `move_score_matrix` and `cube_matrix` (`columns` of top moves; the cube one only shown when `best_move_differs`). The trainer stores them on the item and draws them under the answer in `renderExtras`, coloured like the Anki card. The browser can't compute them: `web.export_pack` has no engine.
